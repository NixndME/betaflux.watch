...
export const BattleStruggleChronicle: React.FC<BattleStruggleChronicleProps> = ({
  ...
  return (
    <div className="space-y-8" id="battle-struggle-section">
      ...
      <div className="p-6 sm:p-8 bg-[#141414] border border-red-900/60 text-white rounded-2xl shadow-xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 bg-red-900/10 rounded-full blur-3xl pointer-events-none -z-0" />
        
        <div className="relative z-10 space-y-4 max-w-5xl">
          <div className="flex items-center gap-2">
            <span className="w-8 h-8 rounded-lg bg-red-950 text-[#FF3366] flex items-center justify-center font-bold text-sm border border-red-800/60 shadow-sm">
              <Flame className="w-4 h-4" />
            </span>
            <span className="text-xs font-bold text-[#FF3366] uppercase tracking-wider bg-red-950/80 px-3 py-1 rounded-full border border-red-800/60">
              The Whistleblower Chronicle &amp; Narrative Truth
            </span>
          </div>

          <h2 className="text-2xl sm:text-4xl font-black tracking-tight text-[#F8FAFC] leading-tight">
          ...
          <div className="pt-2 flex flex-wrap items-center gap-3 text-xs">
            <span className="inline-flex items-center gap-1.5 bg-[#1E1E1E] text-amber-300 px-3 py-1.5 rounded-lg border border-amber-900/60">
              <ShieldAlert className="w-3.5 h-3.5 text-amber-400" />
              Public Purpose: Client Due Diligence &amp; Candidate Protection
            </span>
            <span className="inline-flex items-center gap-1.5 bg-[#1E1E1E] text-red-300 px-3 py-1.5 rounded-lg border border-red-900/60">
          ...
          return (
            <button
              ...
              className={`p-5 rounded-2xl border text-left transition-all flex flex-col justify-between space-y-3 ${
                isSelected
                  ? 'bg-[#181818] border-[#FF3366] ring-1 ring-[#FF3366]/40 shadow-lg shadow-black/60'
                  : 'bg-[#141414] border-[#262626] text-slate-300 hover:bg-[#1C1C1C] hover:border-slate-700'
              }`}
            >
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className={`text-[11px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${
                    isSelected ? 'bg-[#FF3366] text-white' : 'bg-[#262626] text-slate-400'
                  }`}>
                    Chapter {ch.chapterNumber}
                  </span>
                  <span className="text-[11px] font-medium text-slate-400">{ch.period}</span>
                </div>
      ...
      {currentChapter && (
        <div className="p-6 sm:p-9 bg-[#141414] border border-[#262626] rounded-2xl shadow-xl space-y-8 animate-in fade-in duration-200">
          {/* Chapter Top Title Banner */}
          <div className="border-b border-[#262626] pb-6 space-y-2">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <span className="text-xs font-bold text-[#FF3366] bg-red-950/80 px-3 py-1 rounded-md border border-red-800/60 uppercase tracking-wider">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="p-5 bg-[#181818] border border-amber-900/60 rounded-xl space-y-2">
          ...
          <div className="space-y-3 p-5 bg-[#181818] border border-[#262626] rounded-xl">
            ...
            <div className="space-y-2">
              {currentChapter.keyDeceptions.map((dec, idx) => (
                <div key={idx} className="flex items-start gap-2.5 text-xs text-slate-300">
                  <span className="w-5 h-5 rounded-full bg-red-950 text-[#FF3366] border border-red-800/60 flex items-center justify-center font-bold text-[10px] shrink-0 mt-0.5">
                    ✗
                  </span>
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="prose prose-invert max-w-none text-slate-300 text-sm sm:text-base leading-relaxed space-y-4 bg-[#0F0F0F] p-6 rounded-xl border border-[#262626]">
          ...
          <div className="p-5 sm:p-6 bg-gradient-to-r from-red-950 via-slate-900 to-amber-950 border border-amber-900/60 text-white rounded-xl space-y-2">
                ...
                {currentChapter.evidenceRefIds.map((docId) => {
                  ...
                  return (
                    <button
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-xl bg-[#1C1C1C] hover:bg-[#262626] text-slate-200 border border-[#333333] transition-all shadow-sm"
                    >
          ...
          <div className="pt-6 border-t border-[#262626] flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              ...
              className="px-4 py-2.5 text-xs font-semibold rounded-xl bg-[#1C1C1C] hover:bg-[#262626] text-slate-200 border border-[#333] transition-colors"
            >
